...
| `owner`                  | object | always required: `milestone` and optional nullable `issue` are first checked non-blank, bounded, and free of [invisible code points](schema.md#invisible-code-points), then matched against the configured patterns; `workstream` (configured vocabulary) |
...
| `risks`        | risk-record identifiers, free of [invisible code points](schema.md#invisible-code-points), matching the configured `risk_pattern` |
